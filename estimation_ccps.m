...
% copmare vreplace -log(ccp(replace)) with real continuation value

EV=valuefunction(states,trans_matrix,beta,thetareal);
aproximation = -thetareal(1) -log(realccp) + beta*EV(1) ;

check = aproximation-EV;



% estimate the likelihood
[theta13,fval13,exti,output,grad,hessian13]= fminunc(@(thetas) loglike_ccp(thetas,beta,choices,data,ccps,trans_matrix), theta1);
